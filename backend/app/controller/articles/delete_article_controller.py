...
class DeleteArticleController:
    ...
    @staticmethod
    @delete_article_blueprint.route('/api/articles/expert/delete/<int:article_id>', methods=['DELETE'])
    @permission_required('has_expert_permission')
    def delete_article_expert(article_id, **kwargs):
        """Delete an article - by expert (own articles only) or admin (any article)"""
        try:
            # Access current user from permission decorator
            current_user = kwargs.get('current_user')
            if not current_user:
            ...
            print(f"🔧 User {current_user.email} is attempting to delete article {article_id}")
            
            # Use the updated entity method to delete article
            success, status_code, message, article_data = Article.deleteArticle(
                article_id=article_id,
                user_id=current_user.user_id
            )
            ...
            print(f"Error in delete_article_expert controller: {e}")
            return jsonify({
                'success': False,
                'message': f'Error deleting article: {str(e)}'
            }), 500