...
def permission_required(*permission_args):
    """
    Decorator to check if user has required permission(s) and is active.
    Accepts:
        @permission_required("has_admin_permission")
        @permission_required("has_premium_permission", "has_expert_permission")
        @permission_required([])  # no permission check, just authentication
    """
    # Flatten if first arg is a list
    if len(permission_args) == 1 and isinstance(permission_args[0], list):
        permission_names = permission_args[0]
    else:
        permission_names = permission_args

    def decorator(f):
        @wraps(f)
        def decorated_function(*args, **kwargs):
            try:
                print(f"🔐 Starting authentication check for permissions: {permission_names}")
                ...
                if not db_token:
                    ...
                    return jsonify({'success': False, 'error': 'Token not found or expired'}), 401
                ...
                try:
                    ...
                    return jsonify({'success': False, 'error': f'User lookup error: {str(e)}'}), 500
                ...
                try:
                    ...
                    return jsonify({'success': False, 'error': f'User type lookup error: {str(e)}'}), 500

                if not user_type:
                    print("❌ User type not found")
                    return jsonify({'success': False, 'error': 'User type not found'}), 403

                print(f"✅ User type: {user_type.name}")
                print(f"✅ User type dict: {user_type.to_dict()}")

                # ✅ Skip permission check if none are required
                if not permission_names:
                    kwargs['current_user'] = user
                    return f(*args, **kwargs)

                print(f"🔍 Checking permission(s): {permission_names}")
                try:
                    permission_value = any(
                        getattr(user_type, perm, False) for perm in permission_names
                    )
                    print(f"✅ Permission result: {permission_value}")
                except Exception as e:
                    print(f"❌ Error checking permission(s): {e}")
                    return jsonify({'success': False, 'error': f'Permission check error: {str(e)}'}), 500
