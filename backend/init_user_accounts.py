...
with app.app_context():
    ...
    # Sample users to create - FIXED INVALID USER TYPE
    sample_users = [
        {
    ...
    if existing_faqs:
        ...
        for faq in existing_faqs:
            print(f"  - {faq.faq_id}: {faq.question[:50]}...")
        print("⏭️  Skipping FAQ creation (already exist)")
    else:
        print("📋 Creating FAQs...")
        created_faqs = []
        
        # Get the first admin user for FAQ creation
        admin_user = User.query.join(UserType).filter(UserType.name == 'Admin').first()
        admin_user_id = admin_user.user_id if admin_user else 1
        ...
        for faq_item in faq_data:
            ...
            print(f"✅ Prepared FAQ: {faq_item['question'][:50]}...")
        
        # Commit all FAQs at once
        try:
            db.session.commit()
            print("✅ FAQs created successfully!")
            
            # Display created FAQs with their IDs
            for faq in created_faqs:
                print(f"  - Created: {faq.faq_id}: {faq.question[:50]}...")
        except Exception as e:
            db.session.rollback()
            print(f"❌ Failed to create FAQs: {str(e)}")
